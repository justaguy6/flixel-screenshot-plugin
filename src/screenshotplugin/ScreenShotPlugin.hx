...
class ScreenShotPlugin extends flixel.FlxBasic {
    ...
    public static var screenshotKey:FlxKey = #if android || FlxG.android.justReleased.BACK #end;
    public var FilePath:String = lime.system.System.applicationStorageDirectory;
    ...
    private function screenshot():Void {
        ...
        png.position = 0;
        #if (sys || android)
        var path = FilePath + "screenshots/Screenshot " + Date.now().toString().split(":").join("-") + saveFormat;
        if (!Assets.exists("./screenshots/"))
            sys.FileSystem.createDirectory(FilePath + "./screenshots/");
